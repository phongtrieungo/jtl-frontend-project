...
- **Engineering Evaluator:** Seeks evidence of structural discipline, clean boundary enforcement, correct TanStack Router and Query patterns, deliberate Jotai state usage, accessibility compliance, transparent documentation, and a clean BFF contract.
...
- **F1.1: User Creation Form**
  ...
  - **Behavior:** Submits to the User API service. On success, clears the form, shows a success toast, and refreshes the user list.
...
- **F2.2: User-Specific ToDo List (`/todos?userId=...`)**
...
- **F3.1: Active Selected User Atom**
  ...
  - Accessible across the app: user dropdown in the header, ToDo creation form default assignee, and filtering views.
...
- **F4.1: ASP.NET Core .NET 10 Minimal API**
  ...
  - Endpoints:
    ...
    - `POST /api/users`: Creates a user with validation.
    - `GET /api/todos`: Returns todos filtered by `userId`.
    - `POST /api/todos`: Creates a todo for an assignee.
    - `PATCH /api/todos/{id}/toggle`: Toggles todo completion status.
    - `POST /api/chaos/toggle`: Toggles server-side chaos mode.
    - `GET /api/health`: Healthcheck endpoint for frontend auto-discovery.
  - **F4.2: In-Memory Thread-Safe Data Store**
    - `ConcurrentDictionary` storage seeded with demo users and tasks. No external database required.
  - **F4.3: Chaos and Latency Middleware**
    - Injects artificial latency (200-400ms) to emulate realistic network conditions.
    - Intercepts requests when chaos mode is active or when header `X-Simulate-Chaos: true` is supplied, returning `HTTP 500 Simulated Network Failure` on writes.
  - **F4.4: Swagger / OpenAPI Documentation**
    - Interactive Swagger UI available at `/swagger`.

### 3.5 Application Shell & Routing (`apps/web` + TanStack Router)
- **F5.1: Route Structure**
  - `/__root`: Base layout containing top navigation bar, global user switcher, chaos simulation toggle, backend status indicator (BFF connected vs Mock mode), notification container, and outlet.
...
- **F5.2: Type-Safe Navigation & Params**
...
- `services/bff` is a decoupled backend service; communication occurs strictly over HTTP REST contracts.
...
- Dual-mode API client adapter in `packages/shared`.

### 4.3 Form Validation & User Guidance
- Form schemas defined via **Zod**.
- Immediate inline field errors on blur/submit with clear descriptions.
...
- Visible keyboard focus rings (`focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-indigo-600`).
- Screen reader announcements (`role="alert"`, `aria-live="polite"`).

### 4.5 Styling & Design System
- Built with **TailwindCSS** utility classes.
- Palette: **Slate + Indigo** (Linear-like clean SaaS aesthetic).
...
2. **Comprehensive README:**
   ...
   - Justification for monorepo package splits, BFF placement in `services/bff`, and dual-mode API design.
   - Installation and local development instructions (both standalone `pnpm dev` and full-stack `pnpm dev:full`).
...
4. **`ai-journey/` Documentation:**
   - Master AI journey document detailing prompts, architectural decisions, overrides, and tooling.

---

## 6. Explicit Non-Requirements (Out of Scope)
- No heavy external database engines (PostgreSQL, SQL Server) required; in-memory thread-safe storage is used.
- No production CI/CD deployment pipelines.
- No complex authentication or authorization flows.
- No heavy micro-frontend runtimes.
